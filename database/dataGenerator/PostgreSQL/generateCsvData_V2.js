...
const generateProductRecord = (id) => {
  ...
  const protection_plan_id = protection_plan_exists ? getRandomIntInclusive(1, 100) : '';
  ...
  return `${name}|${image}|${link}|${shares}|${price}|${is_prime}|${in_stock}|${giftwrap_available}|${quantity_max}|${seller}|${protection_plan_exists}|${protection_plan_id}`;
};
...
const generateProductDataSet = (records, batch) => {
  ...
  let data = 'name|image|link|shares|price|is_prime|in_stock|giftwrap_available|quantity_max|seller|protection_plan_exists|protection_plan_id\n';
  ...
  const buildNextProductBatch = (batchNumber) => {
    if (batchNumber === totalBatches) {
      ...
      fs.appendFile('productDataSet_V2.csv', data, 'utf8', (err) => {
        if (err) {
  ...
  fs.writeFile('productDataSet_V2.csv', data, 'utf8', (err) => {
    if (err) {
...
const generateProtectionPlanDataSet = (records, batch) => {
  ...
  const buildNextProtectionPlanBatch = (batchNumber) => {
    if (batchNumber === totalBatches) {
      ...
      fs.appendFile('protectionPlanDataSet_V2.csv', data, 'utf8', (err) => {
        if (err) {
  ...
  fs.writeFile('protectionPlanDataSet_V2.csv', data, 'utf8', (err) => {
    if (err) {
      console.log('ERROR inserting header row');
    }
...
generateProductDataSet(100,100);
generateProtectionPlanDataSet(100,100);